...
Be precise about what replaces it, because "`clearCredsCheckBox` still covers it" is *not* true under the new model. Today the list is *derived* from `GetAllServers()`, so `CredentialStore.Clear()` — reached via `CredsHandler.GetCreds()` (`CredsHandler.cs:22-29`) — effectively cleared the list as a side effect. Once the list is persisted independently, that checkbox clears credentials and leaves the full list standing with blank markers. That is arguably the better behavior, but it is a *different* behavior, and stating the equivalence would invite someone to later "fix" the checkbox by making it wipe `Servers` too. Wholesale list clearing is simply gone; removal is per-row.
...
- A `ListBox` of rows, each showing the server name, a read-only "credentials saved" marker where `CredentialStore.Get(server) != null`, and a remove affordance using the **`Button.remove-server` style that already exists** at `App.axaml:313-326` — written during Stage A for exactly this, with a comment at `:309-312` saying it awaits "whichever later stage revisits that model (originally 'Stage C')". Do not write a new style. `localhost` renders without a remove affordance when pinned (§2).

  One interaction to handle: `ListBoxItem:pointerover Button.remove-server` (`App.axaml:324`) raises the button's `Opacity` from `0.5` to `1` **only while its row is hovered**. That is right for a remove affordance, but the undo affordance on a struck-through row must stay legible without hovering — a user scanning what `Done` is about to do should not have to hover each row to see it. Give undo its own class rather than reusing `remove-server`.
...
- **`RadioButton.segment` almost certainly needs FluentTheme hover/pressed neutralization, and verifying that is required rather than conditional.** `App.axaml:252-278` defines the base style, a `/template/ Ellipse` rule, a `/template/ ContentPresenter` rule, and `:checked` — but **no `:pointerover` and no `:pressed`**. Every other interactive class in the file has explicit neutralization for those states: `Button.primary` (`:105`, `:109`), `.secondary` (`:127`), `.link` (`:148`, `:151`), `.tab` (`:180`, `:185`, `:190`, `:195`), `ListBoxItem:selected` (`:296`, `:302`). Given that pattern and the `Button.tab` history below, expect FluentTheme to paint over the checked pill on hover. Treat this as a known gap to close during real-machine verification, not a maybe. When fixing it, follow `feedback_avalonia_fetch_real_template_source`: fetch the real template source for the pinned Avalonia version or copy an already-validated sibling pattern from `App.axaml`. Do not guess. Stage B's `Button.tab` fix needed `Background`, `BorderBrush`, **and** `Foreground` neutralized on `ContentPresenter#PART_ContentPresenter` for both `:pointerover` and `:pressed`; `Background` alone was verified insufficient on real hardware.
...
**Any new test class touching `CAppSettings` must carry `[Collection("GlobalState")]`.** The `CAppSettings.StorePath` seam is necessary but not sufficient: it is a shared mutable static, xUnit parallelizes test classes across threads by default, and the ctor/`Dispose` save-restore pattern guards only *sequential* leakage on one thread — `GlobalStateCollection.cs:7-14` says so explicitly. `CAppSettingsTests.cs:10` is already in that collection, alongside twelve other classes. A new class left outside it will race the existing one and produce an intermittently red suite, which costs far more to diagnose than to prevent.
