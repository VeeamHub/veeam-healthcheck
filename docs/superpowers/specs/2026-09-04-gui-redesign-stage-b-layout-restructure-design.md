...
2. **Tab strip row (new).** Two buttons, `"Ad-hoc Health Check"` and `"Continuous Monitoring"`, mirroring the spike's `AdHocTabButton` / `MonitoringTabButton`. **`Button.tab`/`Button.tab.tab-active` styles do not exist in the real `App.axaml` yet** — Stage A never had tabs to style, so there was nothing to port. This stage adds them, copied from the spike's `App.axaml:82-102` (a compound selector, `Button.tab.tab-active`, not a separate `tab-active`-only class).
...
4. **Bottom bar row (new, fixed).** `Grid ColumnDefinitions="Auto,*,Auto"`: `termsBtn` — progress stack (`pBar` + `progressText`) — `run` button. Same handlers (`AcceptButton_click`, `run_Click`), just relocated out of the scrollable area.
   ...
   **`termsBtn`/`run` follow the active tab; the progress stack does not.** The spike's `SelectTab()` (`MainWindow.axaml.cs:37-45`) hides its *entire* bottom bar, progress included, when switching off the Ad-hoc tab — acceptable there since the spike's "run" is a fake, short timed-delay mockup. The real app's progress bar/text live today in their own row that is unconditionally always visible, and a real health check run can take minutes; a user switching to Continuous Monitoring mid-run must not lose all visibility into it. So Stage B only partially follows the spike here: `termsBtn` (column 0) and `run` (column 2) get `IsVisible` toggled by the active tab, same rationale as the spike (they belong to the ad-hoc workflow; Monitoring has its own action buttons). The progress stack (column 1: `pBar` + `progressText`) keeps today's unconditional visibility regardless of which tab is active. This is a deliberate, confirmed deviation from the spike, not an oversight.
