...
4. **Bottom bar row (new, fixed).** `Grid ColumnDefinitions="Auto,*,Auto"`: `termsBtn` — progress stack (`pBar` + `progressText`) — `run` button. Same handlers (`AcceptButton_click`, `run_Click`), just relocated out of the scrollable area.
   ...
   **Visibility follows the active tab, matching the spike.** The spike's `SelectTab()` (`MainWindow.axaml.cs:37-45`) sets `BottomActionBar.IsVisible = isAdHoc` alongside the two views' visibility — Start/Terms belong to the ad-hoc run workflow, and the Continuous Monitoring tab has its own action buttons (Quick Setup/Setup from VHC/Run Now) that serve the equivalent purpose there. Stage B's tab-switch handler does the same: the bottom bar row's `IsVisible` is `true` only while the Ad-hoc tab is selected. This isn't a new interaction, just carrying over a structural consequence of introducing tabs at all — nothing about what `termsBtn`/`run` do or when they're enabled changes, only when the row containing them is shown.
