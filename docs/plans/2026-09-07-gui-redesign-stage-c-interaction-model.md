...
4. **A fourth review round, ahead of Task 8 (`ServerListCommitter`/`ManageServersDialog`) building on this class, found two real defects in how the constructor's `hasCredentials` predicate was used.**

   - **`Add()` hardcoded `HasCredentials = false` for brand-new rows.** The constructor-injected `hasCredentials` delegate was used only inline while building rows in the constructor — never stored as a field — so `Add()`'s new-row branch set `HasCredentials = false` unconditionally instead of consulting the predicate. This is spec-relevant, not cosmetic: a host can have a stored credential without being in `initial` (e.g. previously removed from the persisted list without its credential being cleaned up — the orphan scenario this whole feature exists to prevent), and typing that hostname into the add box produced a row whose "credentials saved" marker was a lie, and made `HasCredentials` mean something different depending on which branch built the row — a landmine for any future consumer (a Task 8 renderer, or a later change to `Remove()`) that reasonably assumes it means the same thing everywhere. Fixed by storing the constructor parameter as `private readonly Func<string, bool> _hasCredentials;` and consulting it (through the guard below) in `Add()`'s new-row branch too. Note a related but distinct gap this does **not** close, and does not need to: `Remove()` on a row with `IsNewlyAdded == true` drops it from `_rows` entirely rather than staging it, so `Commit()` never sees it and `CredentialsToDelete` cannot include it regardless of `HasCredentials` — an add-then-remove-in-the-same-session sequence for an orphaned-credential host still won't queue a deletion. Changing `Remove()`'s drop-vs-stage behaviour for credentialed newly-added rows is a `ServerListCommitter`-adjacent design decision, not a `HasCredentials`-accuracy bug, and is left for Task 8 to pick up; it doesn't regress anything this fix touches (the add → Done → reopen → remove path already stages correctly, since on reopen the row arrives via `initial` as non-new).
   - **The predicate was invoked unguarded.** Task 8 will pass `name => CredentialStore.Get(name) != null`. `CredentialStore.Get` (`Startup/CredentialStore.cs:142-155`) calls `ProtectedData.Unprotect` with no surrounding try/catch, which can throw `CryptographicException` if the encrypted blob can't be decrypted on this machine/profile (`creds.json` copied to another machine, a recreated Windows profile). Unguarded, that throw would propagate straight out of a button-click handler in Task 8's planned dialog, making `ManageServersDialog` — the one surface that could remove the offending host — permanently unopenable. Fixed with a `private bool SafeHasCredentials(string name)` helper that wraps the call in try/catch and defaults to `true` on any exception, used in both the constructor's row-building loop and `Add()`'s new-row branch (the single place the guard logic now lives). Defaulting to `true` keeps the "credentials saved" marker honest for a row that has *something* on disk, and if the row is later removed, routes it through `CredentialStore.Remove` — which never decrypts anything, so it actually cleans the corrupted entry up.

     Also reworded the constructor's `HasCredentials` comment: it previously said credentials are "Snapshotted once at construction," which stopped being accurate once `Add()` could call the same predicate later for a newly-typed name.

     Four new tests: `Add_NameWithExistingCredentialsNotInInitial_ReportsHasCredentialsTrue`, `Constructor_HasCredentialsPredicateThrowsCryptographicException_DefaultsToTrueAndDoesNotThrow`, `Add_HasCredentialsPredicateThrowsCryptographicException_DefaultsToTrueAndDoesNotThrow`, `Add_NullName_ReturnsInvalid`. (The latter two were renamed in item 5 below when the catch they exercise was narrowed; same tests, more precise names.)

     Mutation testing, each reproduced and confirmed to fail the specific new test before being reverted: (1) hardcoding `HasCredentials = false` back into `Add()`'s new-row branch failed `Add_NameWithExistingCredentialsNotInInitial_ReportsHasCredentialsTrue`; (2) deleting the try/catch entirely (letting the exception propagate) failed both credential-predicate-throws tests with the simulated exception itself; (3) changing `catch { return true; }` to `catch { return false; }` — same guard, wrong default — also failed both of those tests, proving the specific default value is load-bearing and not just "a catch block exists somewhere." All three mutations were reverted and the full suite re-confirmed green afterward.

     Also made two small, low-risk test-quality improvements flagged by the same review pass: `Rows_ReflectInitialListPinningAndCredentials` now indexes rows by `.Single(r => r.Name == ...)` instead of positionally (`Rows[0]`/`Rows[1]`), and `Add_CoversItsFourOutcomes`'s final assertion now explicitly asserts `vbr01` is present via `Assert.Contains` before checking `IsPendingRemoval`, rather than relying on `.Single(...)` to throw implicitly if it were missing.

5. **A code-quality review of item 4's fix (dispatched independently, after a separate spec-compliance review had already passed it) found the `catch { return true; }` guard broader than its own stated justification.** The comment justified it purely in terms of `CryptographicException` from DPAPI on a corrupted-but-present credential blob, but a bare `catch` also swallows an unrelated bug in a caller's predicate — and that is not inert further downstream. If a predicate throws for a host with *no* stored credential at all, `SafeHasCredentials` still defaults to `true`; if the user removes that row, `Commit()` stages it in `CredentialsToDelete`, `ServerListCommitter.Execute()` (Task 8) calls `CredentialStore.Remove`, which returns `false` for a host with nothing to remove, and `ServerListCommitter`'s own reinstatement logic (item added when Task 8 was restructured — see `a1b639b`/`6ae299c`) then keeps that host in the persisted list. Net effect: an explicit user removal silently undone by an unrelated bug, two review rounds later, in a class neither round touches — the same "two things independently fail-safe and the combination isn't safe" shape this branch has hit before (Task 3's seed burning itself).

   Narrowed the catch to `catch (CryptographicException)`. Renamed the two guard tests to `...ThrowsCryptographicException...` to match, and added `Constructor_HasCredentialsPredicateThrowsUnrelatedException_PropagatesRatherThanDefaulting`, asserting `Assert.Throws<InvalidOperationException>(...)` for a predicate that throws something other than `CryptographicException`. Mutation-tested: reverting to a bare `catch` was reproduced and confirmed to fail exactly the new unrelated-exception test (19 passed, 1 failed), then reverted and the full suite re-confirmed green.

   Also reworded the `SafeHasCredentials` comment: "permanently unopenable" is precise for the constructor call site (runs on dialog open) but overclaimed for `Add()`'s call site (dialog is already open by then; the real failure mode there is a crashed in-session interaction, not an unopenable dialog) — now qualified accordingly.

**Suite after Task 4: 903 passed, 0 failed, 12 skipped** (verified directly). Baseline for Task 5.
...
**Executed: `a7ae144`** (dialog, as specified — no deviation from the plan's Step 3/4 code) **+ `33a9ca6`** (this correction) **+ `c9fc0835`/`4fdf458a`** (item 2, a later correction found before Task 9). `ManageServersDialog.axaml`/`.axaml.cs` shipped byte-identical to the plan; the defect was in the plan's own design of the `SettingsSaved == false` path, not a transcription error.
...
2. **The `SettingsSaved == true` path — the one item 1 left untouched — had its own silent-reinstatement bug, found after Task 8 shipped and was twice reviewed, before starting Task 9.** Trace: a user stages removal of a credentialed host and confirms; `CredentialStore.Remove` returns `false` for a genuine reason (disk/permissions, or any other real failure `CredentialStore.Remove` swallows into a bool); `ServerListCommitter.Execute` reinstates that host into `ServersPersisted` per its by-design recovery behaviour (see Step 1's comment and the Task 8 Steps 1-2 corrections above); `CAppSettings.SetServers` then succeeds, as it usually will, because the reinstatement exists precisely so a failed credential deletion doesn't also corrupt the settings list. `doneBtn_Click` took the `outcome.SettingsSaved == true` branch, which only logged each entry of `outcome.FailedCredentialRemovals` via `CGlobals.Logger.Error` and then called `Close(true)` unconditionally. `Close(true)` tells `manageServersBtn_Click` (Task 12) that changes committed cleanly, so it calls `CAppSettings.LoadOrSeedServers(...)` and repopulates `serverSelector` from the freshly-read persisted list — which now contains the reinstated host. The host the user explicitly asked to remove reappeared in the picker immediately, with the only explanation in a log file the user will never open. Same failure shape as Task 3's seed burning itself and item 5 of the Task 4 corrections above: two things that are each individually correct (the reinstatement recovery design; a `Close(true)` that's honest about the settings write) combine into an explicit user action being silently undone. It slipped past both review rounds on the dialog because both were focused on the credential-deletion-failure path itself and the `SettingsSaved == false` retry-disable fix in item 1, not on what the dialog tells the user when a reinstatement happens but everything else succeeds.

   Fixed by adding a check between the existing `SettingsSaved == false` return and the final `Close(true)`: if `outcome.FailedCredentialRemovals.Count > 0` (only reachable here since the `false` case already returned), show `GuiManageServersPartialFailureTitle`/`GuiManageServersPartialFailureBody` (two new localization keys — deliberately not a reuse of `GuiManageServersSaveFailed`, which would be factually wrong here since the list did save) naming the affected host(s) via `string.Join(", ", outcome.FailedCredentialRemovals)`, then still call `Close(true)` — the settings list genuinely saved and Task 12's repopulate should still happen; the fix informs the user, it doesn't block the close. Also disabled `doneBtn` and `cancelBtn` across the new `await`, with no re-enable, for the same reason as the existing confirm-dialog guard in Step 4: `AvaloniaUiNotifier` disables `VhcGui` (`AvaloniaHost.MainWindow`), not this dialog, so without the guard a Cancel click during the message would resolve the caller's `ShowDialog<bool>` with `false` — skipping `manageServersBtn_Click`'s repopulate for a commit that DID succeed, leaving the picker stale — and then let this suspended handler `Close(true)` a window that Cancel had already closed. Caught by review before this correction shipped, not by a running dialog (still no test coverage for this file).
...
4. **All three `doneBtn_Click` button-disable guards (confirm, settings-save-failed, partial-failure) blocked clicks on `doneBtn`/`cancelBtn` but not the OS title-bar close button (or Alt+F4), which drives `Window.CloseCore` through a separate path with no `Button` in it to disable.** `AvaloniaUiNotifier` disables the *main* window during these awaits, not `ManageServersDialog` itself, so the dialog — including its native close button — stays fully interactive the whole time; this was already known and commented for the in-app buttons, but the native close button was an unguarded second door into the exact hazard those comments describe. Traced concretely for the confirm-guard path: click the OS close button while the removal confirm is up. `ShowDialog<bool>` resolves `false` immediately — `manageServersBtn_Click` (Task 12) skips its repopulate, believing nothing committed — while the confirm dialog itself, owned by the main window, stays on screen. Answering "yes" on it resumes the suspended handler, which proceeds to delete credentials and write settings anyway, after the dialog already reported no commit happened. A code-quality review of item 2 first surfaced this race but characterized it as low-severity UI staleness; a follow-up review traced this specific path through to the end and found it was actually destructive-action-after-a-reported-cancel, the same failure shape as items 1-2 and Task 3's seed burning itself.

   Fixed with a `private bool _busy` field set around all three `CGlobals.Notifier` awaits and a `protected override void OnClosing(WindowClosingEventArgs e)` that sets `e.Cancel = true` when `_busy && !e.IsProgrammatic`. Verified against the actual Avalonia 11.3.20 source (fetched directly, not assumed, per this branch's established rule about guessing Avalonia API shape) that `Window.Close()`/`Close(object)` route through `CloseCore` with `isProgrammatic: true`, while the native close path (`HandleClosing`, invoked from `IWindowImpl.Closing`) passes `isProgrammatic: false` — so `WindowClosingEventArgs.IsProgrammatic` reliably tells this class's own `Close(...)` calls apart from a user-initiated close regardless of `_busy`'s exact value at that instant, and a programmatic close is never blocked. No test added, same reason as items 1-3 — this is Avalonia UI code with no coverage in this codebase. Task 14's Windows verification checklist should add: while the removal confirm (or either error notice) is on screen, try the OS title-bar close button and Alt+F4; both should be inert until the dialog's own flow resolves.
...
### Corrections applied after review — Task 9 as built

**Executed: `0cc7a65f`** (feature, as specified) **+ `c4c808ac`/`59e8da91`/`2ab7c6eb`** (comment-sweep gaps, folded into the plan text above rather than kept as a separate narrative) **+ `2d56a75a`** (this correction). The XAML/handler-rename work shipped exactly as written; the defect was in the plan's own `termsCheckBox_Checked`/`_Unchecked` design, not a transcription error.

1. **Nothing prevented `termsCheckBox_Checked` from being re-entered while its own `AcceptTerms()` await was pending, and neither its nor a later resolution re-checked whether anything had superseded it in the meantime.** A code-quality review traced two concrete, reachable consequences: (a) a fast uncheck-then-recheck (or keyboard auto-repeat) during the await's dispatcher-queue delay starts a second, overlapping accept flow — two confirm dialogs, two continuations racing to write `run.IsEnabled` with no coordination, whichever resolves last winning regardless of the checkbox's actual state by then; (b) `Import_click` does not wait on terms acceptance at all and can race ahead of an in-flight accept flow, calling `DisableButtons()` while that flow is still pending — a stale continuation resolving afterward would silently re-enable `run.IsEnabled` (or the checkbox itself) after a run/import had already started, and `run_Click` has no guard against starting a second, concurrent collection if the user then clicks Run, corrupting whatever shared `CGlobals` state both writes touch. The same review found the plan's `_suppressTermsHandler` guard, as specified, was actually inert for its stated purpose — `run.IsEnabled = accepted` already runs before the synthetic `Unchecked` it guards against, so removing the guard entirely would not have changed the decline-path's observable behavior at all. A follow-up review confirmed it is **still** inert after item 2's logging fix below: that fix deliberately logs `"Accept = false"` *before* the guard check specifically so it fires unconditionally, so `_suppressTermsHandler` continues to gate nothing but the already-redundant `run.IsEnabled = false` write. It is kept rather than removed only because a future change to `termsCheckBox_Unchecked` (e.g. a side effect placed *after* the guard check instead of before it) could make it load-bearing again, and removing it now would just mean re-adding it later.

   Fixed with two additions beyond the plan's original design: `_termsAcceptInFlight` blocks re-entrancy, and `termsCheckBox.IsEnabled = false` for the await's duration closes the double-toggle path at the source. A new `_guiLockedForRun` flag (set once, inside `DisableButtons()`, never cleared) makes explicit a one-way-ratchet invariant that was already true but implicit in this codebase — every control `DisableButtons()` touches, other than `run.IsEnabled` on failure via `ReportRunFailure`, stays disabled for the rest of the process's life once a run/import starts, since success calls `Environment.Exit(0)` outright. `termsCheckBox_Checked`'s continuation checks `_guiLockedForRun` immediately after its `await` and, if set, treats its own result as stale: it returns without writing `run.IsEnabled` and without re-enabling the checkbox in the `finally`.

2. **`termsCheckBox_Unchecked` logged nothing, unlike every other `Checked`/`Unchecked` pair in this file** (`pdfCheckBox`, `explorerShowBox`, `clearCredsCheckBox`, `RescanBox` all log both edges). Fixed by logging `"Accept = false"` unconditionally at the top of `termsCheckBox_Unchecked`, including while `_suppressTermsHandler` is set — "terms are no longer accepted" is equally true whether the call came from a real user click or the programmatic revert on decline, so there is exactly one log line for that fact rather than needing a second call site inside the decline branch.

   No test added — this is Avalonia UI code with no coverage in this codebase, the same pattern as every dialog/handler task on this branch. Task 14's Windows verification checklist should add: rapidly uncheck-then-recheck the terms box before the confirm modal appears, and separately, click Import immediately after checking terms and before answering its confirm modal — in both cases, confirm Run never becomes clickable as a side effect of the stale flow resolving after the GUI has moved on.

**Suite after Task 9: 908 passed, 0 failed, 12 skipped** (verified directly; this task added no new tests). Baseline for Task 10.
...
1. **`days7`'s `IsChecked="True"` unconditionally overwrote a `/days:N` CLI value with `7` the instant the window was constructed — a real regression versus the `ComboBox` this task replaced.** The plan's own Step 2 comment asserted this was safe "because `CGlobals.reportDays` already defaults to 7," which only covers the no-CLI-override case. Traced by an independent code-quality review against `CArgsParser.cs`'s `/days:7|30|90|12` cases (all of which run and set `CGlobals.ReportDays` before `LaunchUi` ever constructs `VhcGui`): the old `ComboBox_SelectionChanged`'s `daysSelector == null` guard suppressed its own equivalent initial firing, because `daysSelector`'s named field was not yet assigned to itself at the moment its own `SelectedIndex="0"`-driven `SelectionChanged` fired during `InitializeComponent()` — but a `RadioButton`'s `sender` on its own `Checked` event is always non-null (it *is* the object raising the event), so the same guard shape does not carry over to the new control. `/gui /days:90` silently became `/gui /days:7` the moment the window opened; `/gui /days:12` (no matching pill even before this task) is a narrower pre-existing limitation that this task did not introduce or worsen.
   ...
   Fixed by capturing `CGlobals.ReportDays` before `InitializeComponent()` runs, then resyncing after: for `30`/`90`, checking the matching pill (which re-fires `PeriodRadio_Checked`, correctly restoring the value); for `7`, doing nothing (already correct); for anything else (e.g. `12`), restoring the captured value via `SetReportDays` (not a raw assignment — see follow-up below), since no pill can represent it — the same cosmetic "value preserved, wrong pill shown" outcome the old `ComboBox`'s `SelectedIndex="0"` default already produced for that case.

   A review of this fix (`71c40c5b`) caught one more thing: the `default` branch's first cut restored `CGlobals.ReportDays` via a raw assignment, leaving the log's last "Interval set to N" entry saying `7` (from `SetReportDays(7)` firing during `InitializeComponent()`) even though the actual value ended up something else. Fixed in `918d43a1` by routing that restoration through `SetReportDays` itself, so the log gets a correcting entry with the real value.
