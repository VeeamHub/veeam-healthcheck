...
**Follow-up (2026-08-23): full-pipeline production-build comparison, a 4th environment.** Beyond the scratch script, the user ran the actual shipped tool (old release `v3.0.1.193` vs. this PR's build) against a real 21-job environment and compared the compiled JSON reports directly. 20 of 21 jobs matched exactly or were explained 0/0 fixes consistent with the three scratch-script labs. One job (`VBR Managed Agents - Windows`) showed a real, non-zero-to-smaller-non-zero change (76.47GB → 40.63GB SourceSize) — traced via a follow-up scratch-script run against the same environment to a Tier-2-suppression case: a second machine (`WindowsAgent08`) with exactly one restore point ~5 months stale, name-matched to the job but correctly suppressed because the job already had Tier-1 matches from an actively-backing-up machine (`WindowsAgent07`). Arithmetic reconciles exactly (old RPCount 8 = new RPCount 7 + 1 suppressed) — this is ADR 0021's gating tradeoff firing as designed, not a defect in this change.

This surfaced a real, pre-existing observability gap: `Get-VhcJob.ps1` doesn't track or expose which restore points get Tier-2-suppressed (or why) — it's silently folded into the "unmatched" log arithmetic. Filed as a scope addition to [#192](https://github.com/VeeamHub/veeam-healthcheck/issues/192) ("Orphaned Backups" report section) rather than a blocker for this PR: #192 should cover two categories — (1) its original scope of restore points that never resolve to any job, and (2) superseded chains for a still-active job that resolve by name but get excluded by Tier 2's suppression gate. Not implemented as part of this branch.
